    ...
    <main className="max-w-5xl mx-auto px-6 py-10">
      {polls.length === 0 ? (
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-bold text-warm-900">Polls</h2>
          {polls.map((poll) => {
            ...
            return (
              <div
                key={poll.id}
                className="rounded-xl border border-warm-200 bg-white p-5 hover:border-warm-300 transition-colors"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2.5">
                      <Link
                        href={`/poll/${poll.id}`}
                        className="text-base font-semibold text-warm-900 hover:text-accent transition-colors truncate"
                      >
                        {poll.title}
                      </Link>
                    ...
                    <div className="mt-2.5 flex items-center gap-3">
                      <div className="w-32 h-1.5 rounded-full bg-warm-100 overflow-hidden">
                        <div
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <Link
                      href={`/poll/${poll.id}`}
                      className="rounded-lg px-3 py-1.5 text-sm font-medium text-warm-500 hover:text-warm-900 hover:bg-warm-100 transition-colors"
                    >
                      Results
                    </Link>
                    <Link

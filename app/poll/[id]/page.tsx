    ...
    <main className="max-w-6xl mx-auto px-6 py-8 space-y-6">
      ...
      <div className="flex items-start justify-between gap-6">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-warm-900 dark:text-warm-100">{p.title}</h1>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Link href={`/poll/${id}/edit`} className="rounded-lg px-3 py-1.5 text-sm font-medium text-warm-500 hover:text-warm-900 dark:hover:text-warm-100 hover:bg-warm-100 dark:hover:bg-warm-800 transition-colors">
            Edit
          </Link>
          <Link href={`/poll/${id}/invite`} className="rounded-lg px-3 py-1.5 text-sm font-medium text-warm-500 hover:text-warm-900 dark:hover:text-warm-100 hover:bg-warm-100 dark:hover:bg-warm-800 transition-colors">
            Send Invites
          </Link>
      ...
      {totalCount > 0 && (
        <details className="group">
          <summary className="flex items-center gap-3 cursor-pointer text-sm text-warm-500 hover:text-warm-700 dark:hover:text-warm-300 transition-colors select-none">
            <span className="tabular-nums font-medium">{votedCount}/{totalCount} voted</span>
            <div className="w-24 h-1.5 rounded-full bg-warm-200 dark:bg-warm-800 overflow-hidden">
              <div
            ...
            <span className="text-warm-300 dark:text-warm-600 group-open:rotate-90 transition-transform">&#9654;</span>
          </summary>
          <div className="mt-3 rounded-xl border border-warm-200 dark:border-warm-800 bg-white dark:bg-warm-900 overflow-hidden">
            <div className="max-h-64 overflow-y-auto divide-y divide-warm-100 dark:divide-warm-800/50">
              {p.members.map((m) => {
                ...
                return (
                  <div key={m.id} className="flex items-center gap-3 text-sm px-4 py-2.5">
                    <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${voted ? "bg-emerald-500" : "bg-warm-300 dark:bg-warm-600"}`} />
                    <a href={`/members/${encodeURIComponent(m.email)}`} className="text-warm-700 dark:text-warm-300 w-32 truncate font-medium hover:text-accent transition-colors">{m.name}</a>
                    ...
                    {voted && <span className="text-emerald-600 dark:text-emerald-400 text-xs font-medium ml-auto">voted</span>}
                  </div>
                );
              })}
            </div>

    ...
    <form onSubmit={handleSubmit} className="mt-3 space-y-3">
      <div className="flex items-center gap-3">
        ...
        <button type="button" onClick={() => fileRef.current?.click()}
          className="rounded-lg border border-warm-200 bg-white px-3 py-2 text-sm font-medium text-warm-600 hover:bg-warm-50 transition-colors">
          Upload CSV
        </button>
      ...
      <textarea value={csv} onChange={(e) => handleTextChange(e.target.value)} rows={3}
        ...
        className="w-full rounded-lg border border-warm-200 bg-white px-3 py-2.5 text-sm font-mono text-warm-900 placeholder:text-warm-400 focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent" />
      {preview.length > 0 && (

...
export function NewPollForm({ action }: { action: (formData: FormData) => Promise<{ id: string }> }) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="w-full rounded-xl border border-dashed border-warm-300 py-4 text-sm font-medium text-warm-400 hover:border-accent hover:text-accent transition-colors"
      >
        + New Poll
      </button>
    );
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-warm-200 bg-white p-6 space-y-4">
      <h2 className="text-base font-bold text-warm-900">New poll</h2>
      ...
      <div>
        ...
        {slots.length > 0 && (
          <div className="space-y-1.5 mb-3">
            {slots.map((s, i) => (
              <div key={i} className="flex items-center gap-3 text-sm bg-warm-50 rounded-lg px-3 py-2.5">
                <span className="text-sm font-medium text-warm-400 tabular-nums w-5">{i + 1}.</span>
                <span className="font-medium text-warm-700">{s.label}</span>
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={addSlot} className="shrink-0 rounded-lg bg-warm-100 px-3 py-2.5 text-sm font-medium text-warm-600 hover:bg-warm-200 transition-colors">
            Add
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        <button type="button" onClick={() => setOpen(false)} className="text-sm text-warm-400 hover:text-warm-600 transition-colors">
          Cancel
        </button>
      </div>
    </form>
